...
class Analysis():
    def GetMood(sentence):
        ...
        if total==0: total=1
        result+="Happy Percentage: "
        result +=str(int((hs/total) * 100))+" percent\n"
        result+="Sad Percentage: "
        result +=str((int((ss/total) * 100)))+" percent\n"
        result+="Angry Percentage: "
        result +=str(int((angs/total) * 100))+" percent\n"
        result +="Tired Percentage: "
        result +=str(int((tirs/total) * 100))+" percent\n"
        return result
    def CFC(word,array):
...
def main():
    print(Analysis.GetMood(input("Please enter a sentence:\n")))
